...
type LayoutRouteId = RouteId | "/" | "/(app)/dashboard" | "/(app)/profile" | "/(app)/profile/[username]" | "/(app)/search" | "/(auth)/forgot-password" | "/(auth)/login" | "/(auth)/logout" | "/(auth)/register" | "/(auth)/reset-password" | "/(auth)/verify-email" | "/(auth)/verify-email-notice" | null
