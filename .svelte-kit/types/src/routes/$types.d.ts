...
type LayoutRouteId = RouteId | "/" | "/(app)/dashboard" | "/(auth)/forgot-password" | "/(auth)/login" | "/(auth)/logout" | "/(auth)/register" | "/(auth)/reset-password" | "/(auth)/verify-email" | "/(auth)/verify-email-notice" | null
