...
export const actions = {
  ...
  updateGaming: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) {
      return fail(401, { message: 'Unauthorized' });
    }
  ...
  updatePrivacy: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) {
      return fail(401, { message: 'Unauthorized' });
    }
  ...
  updateNotifications: async ({ request, locals }: import('./$types').RequestEvent) => {
    if (!locals.user) {
      return fail(401, { message: 'Unauthorized' });
    }
